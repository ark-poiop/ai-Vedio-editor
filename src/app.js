...
(() => {
  ...
  function handleInspectorChange(target) {
    ...
    if (selection.kind === 'clip') {
      ...
      clip[key] = key === 'trackId' || key === 'transition-type' ? target.value : Number(target.value);
      ...
      text[key] = key === 'text' || key === 'color' || key === 'fontFamily' || key === 'trackId'
        ? target.value
        : key === 'background'
          ? `${target.value}bb`
          : Number(target.value);
