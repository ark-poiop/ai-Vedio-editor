...
(() => {
  ...
  function applyBoxWidthToAll() {
    ...
    if (!source) return;
    const boxWidth = source.boxWidth || 92;
    commit((project) => {
  ...
  function addText() {
    const id = uid();
    commit((project) => { project.texts.push({ id, role: 'text', text: '텍스트를 입력하세요', start: state.playhead, end: Math.min(project.duration, state.playhead + 4), x: 50, y: 76, fontSize: 56, fontWeight: 800, fontFamily: 'sans-serif', opacity: 1, boxWidth: 92, color: '#ffffff', background: '#00000099', align: 'center', keyframes: [] }); return project; });
