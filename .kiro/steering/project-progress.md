...
8. MP4 capability 확인 → 자산 업로드 → Job 진행률·취소 → 결과 다운로드
9. FFmpeg 미지원 환경에서 MP4 비활성 이유와 WebM fallback 확인
10. WebM 내보내기 완료
...
## 다음 개발 우선순위
1. 긴 영상에서 숏폼 후보 자동 생성
2. 실제 상용 STT Provider 선택 및 webhook 연결
3. 렌더 Job 영속 저장소·큐·인증·보관 정책과 수평 확장
4. 리프레임 다중 피사체 전환·고급 추적과 분석 Worker 최적화
5. 침묵 감지 파라미터 프리셋과 대용량 미디어용 Worker 최적화
